...
const toastVariants = cva(
  "group pointer-events-auto relative flex w-full items-center justify-between space-x-4 overflow-hidden rounded-md border p-4 pr-8 shadow-lg transition-all data-[swipe=cancel]:translate-x-0 data-[swipe=end]:translate-x-[var(--radix-toast-swipe-end-x)] data-[swipe=move]:translate-x-[var(--radix-toast-swipe-move-x)] data-[swipe=move]:transition-none data-[state=open]:animate-in data-[state=closed]:animate-out data-[swipe=end]:animate-out data-[state=closed]:fade-out-80 data-[state=closed]:slide-out-to-right-full data-[state=open]:slide-in-from-top-full data-[state=open]:sm:slide-in-from-bottom-full",
  {
    variants: {
      variant: {
        default: "border bg-toast-background text-toast-foreground border-toast-border",
        destructive:
          "destructive group border-destructive/50 bg-toast-background text-toast-foreground border-toast-border",
      },
    },
    defaultVariants: {
      variant: "default",
...
>(({ className, variant, children, ...props }, ref) => {
  return (
    <ToastPrimitives.Root
      ref={ref}
      className={cn(toastVariants({ variant }), "h-12 sm:h-14 px-[10px]", className)}
      {...props}
    >
        <div className="flex gap-2 items-center">
            <div className={cn(
                "bg-white/5 backdrop-blur-xl p-1 rounded-lg",
                variant === 'destructive' ? "text-toast-destructive" : "text-primary"
            )}>
                 {variant === 'destructive' ? (
                     <svg
...
>(({ className, ...props }, ref) => (
  <ToastPrimitives.Close
    ref={ref}
    className={cn(
      "absolute right-1 top-1/2 -translate-y-1/2 rounded-md p-1 text-gray-600 hover:bg-white/10 transition-colors ease-linear focus:outline-none focus:ring-2 focus:ring-ring group-hover:opacity-100",
