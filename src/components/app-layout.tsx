...
import { processTextAction, analyzeToneAction, summarizeDocumentAction, hummanizeAction, aiDetectionAction, plagiarismCheckAction } from '@/lib/api';
...
export function AppLayout() {
  ...
  const handleGenericAction = async <T,>(action: () => Promise<{ success: boolean; data?: T; error?: unknown }>): Promise<T> => {
    try {
    ...
    <SidebarProvider>
      <ModelProvider>
        <div className="flex min-h-screen w-full">
          <HistorySidebar
            history={history} isLoaded={isLoaded}
          ...
          <SidebarInset className="flex-1 flex flex-col min-w-0">
            ...
            <main className="flex-1 flex flex-col">
              <ExpanderView
                ...
                onProcessText={async (params) => handleGenericAction(() => processTextAction(params))}
                onAnalyzeTone={async (params) => handleGenericAction(() => analyzeToneAction(params))}
                onSummarizeDocument={async (params) => handleGenericAction(() => summarizeDocumentAction(params))}
                onHummanize={async (params) => handleGenericAction(() => hummanizeAction(params))}
                onAIDetection={async (params) => handleGenericAction(() => aiDetectionAction(params))}
                onPlagiarismCheck={async (params) => handleGenericAction(() => plagiarismCheckAction(params))}
